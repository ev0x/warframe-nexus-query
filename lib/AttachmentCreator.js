          ...
          if (marketComponents.length > 0 && marketComponents[0].prices.soldPrice) {
            marketComponents.forEach((marketComponent) => {
              if (!found && (marketComponent.name.indexOf(component.name) > -1 ||
                ...
                attachment.fields.push({
                  name: '_ _',
                  value: `Supply: **${0}** units (${String((parseFloat(nexusComponent.supply.percentage || 0) * 100).toFixed(2))}%) ` +
                      `- Demand: **${0}** units (${String((parseFloat(nexusComponent.demand.percentage || 0) * 100).toFixed(2))}%)`,
                });
              }
            });
          } else {
  ...
  async mapNexusColors(components) {
    ...
    try {
      components.forEach(async (component) => {
        ...
        const imgUrl = `https://nexushub.co/img/warframe/items/${encodeURIComponent(component.title)}.png`;
        const options = {
          url: imgUrl,
